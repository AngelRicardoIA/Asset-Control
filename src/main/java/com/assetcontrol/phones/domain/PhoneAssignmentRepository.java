...
public interface PhoneAssignmentRepository extends JpaRepository<PhoneAssignment, Long> {
    ...
    long countByTypeAndReturnedAtIsNullAndDueAtBefore(PhoneAssignmentType type, LocalDate today);

    @EntityGraph(attributePaths = {"phone", "person"})
    List<PhoneAssignment> findTop5ByTypeAndReturnedAtIsNullAndDueAtBeforeOrderByDueAtAscIdAsc(
            PhoneAssignmentType type, LocalDate today
    );
